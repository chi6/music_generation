...
class PerformanceRNN(nn.Module):
    def __init__(self, event_dim,  init_dim, hidden_dim,
        ...
        self.concat_input_fc = nn.Linear(self.event_dim, self.input_dim)
        ...
        # variable for drum
        self.inithid_fc1 = nn.Linear(init_dim, 1 * hidden_dim)
        self.inithid_fc_activation1 = nn.Tanh()

        self.event_embedding1 = nn.Embedding(event_dim, event_dim)
        self.concat_input_fc1 = nn.Linear(self.event_dim, self.input_dim)
        self.concat_input_fc_activation1 = nn.LeakyReLU(0.1, inplace=True)

        self.gru1 = nn.GRU(self.input_dim, self.hidden_dim,
                          num_layers=1, dropout=gru_dropout)
        self.output_fc1 = nn.Linear(hidden_dim * 1, self.output_dim)
        self.output_fc_activation1 = nn.Softmax(dim=-1)
    ...
    def _initialize_weights(self):
        ...
        nn.init.xavier_normal_(self.event_embedding1.weight)
        nn.init.xavier_normal_(self.inithid_fc1.weight)
        self.inithid_fc1.bias.data.fill_(0.)
        nn.init.xavier_normal_(self.concat_input_fc1.weight)
        nn.init.xavier_normal_(self.output_fc1.weight)
        self.output_fc1.bias.data.fill_(0.)

    def _sample_event(self, output, output_drum, greedy=True, temperature=1.0):
        if greedy:
            return output.argmax(-1), output_drum.argmax(-1)
        else:
            output = output / temperature
            probs = self.output_fc_activation(output)
            output_drum = output_drum / temperature
            probs_drum = self.output_fc_activation(output_drum)
            return Categorical(probs).sample(), Categorical(probs_drum).sample()

    def forward(self, event, event_drum, hidden=None, hidden1 = None):
        # One step forward
        ...
        event_drum = self.event_embedding1(event_drum)

        input = self.concat_input_fc(event)
        input = self.concat_input_fc_activation(input)

        input1 = self.concat_input_fc1(event_drum)
        input1 = self.concat_input_fc_activation1(input1)
        ...
        _, hidden1 = self.gru1(input1, hidden1)
        output1 = hidden1.permute(1, 0, 2).contiguous()
        output1 = output1.view(batch_size, -1).unsqueeze(0)
        output1 = self.output_fc1(output1)
        return output,  output1, hidden,hidden1
    
    def get_primary_event(self, batch_size):
        return torch.LongTensor([[self.primary_event] * batch_size]).to(device)
    
    def init_to_hidden(self, init, init1):
        ...
        # [batch_size, init_dim]
        batch_size = init1.shape[0]
        out1 = self.inithid_fc1(init1)
        out1 = self.inithid_fc_activation1(out1)
        out1 = out1.view(1, batch_size, self.hidden_dim)
        return out, out1

    
    def generate(self, init, init1, steps, events=None, events_drum = None, greedy=1.0,
        ...
        event_drum = self.get_primary_event(batch_size)

        hidden, hidden1 = self.init_to_hidden(init, init1)
        ...
        for step in step_iter:
            output, output1, hidden, hidden1 = self.forward(event=event, event_drum=event_drum,
                                          hidden=hidden, hidden1=hidden1)

            use_greedy = np.random.random() < greedy
            event, event_drum = self._sample_event(output, output1, greedy=use_greedy,
                                       temperature=temperature)

            if output_type == 'index':
                outputs.append(event)
                drum_outputs.append(event_drum)
            elif output_type == 'softmax':
                outputs.append(self.output_fc_activation(output))
                drum_outputs.append(self.output_fc_activation1(output1))
            elif output_type == 'logit':
                outputs.append(output)
                drum_outputs.append(output1)
            else:
                assert False
            ...
            if use_teacher_forcing and step < steps - 1: # avoid last one
                if np.random.random() <= teacher_forcing_ratio:
                    ...
                    event_drum = events_drum[step].unsqueeze(0)
        
        return torch.cat(outputs, 0),torch.cat(drum_outputs, 0)
