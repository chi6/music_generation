...
from pytorch_melody_rnn.model import PerformanceRNN
...
def get_options():
    ...
    parser.add_option('-d', '--dataset',
                      dest='data_path',
                      type='string',
                      default='../dataset/processed/classical/')


    parser.add_option('-m', '--drum_dataset',
                      dest='drum_data_path',
                      type='string',
                      default='../dataset/processed/drums/')
...
drum_data_path = options.drum_data_path
...
def load_dataset():
    global data_path, drum_data_path
    dataset_drum = Dataset(drum_data_path, verbose=True)
    dataset = Dataset(data_path, verbose=True)
    dataset_size = len(dataset.samples)
    assert dataset_size > 0
    return dataset, dataset_drum
...
dataset, dataset_drum = load_dataset()
#print(dataset)
...
try:
    ...
    drum_batch_gen = dataset_drum.batches(batch_size, window_size, stride_size)
    for iteration, ((events, controls), (events_drum, controls_drum)) in enumerate(zip(batch_gen, drum_batch_gen)):
        if use_transposition:
            offset = np.random.choice(np.arange(-6, 6))
            events, controls = utils.transposition(events, controls, offset)
            events_drum, controls_drum = utils.transposition(events_drum, controls_drum, offset)

        events = torch.LongTensor(events).to(device)
        events_drum = torch.LongTensor(events_drum).to(device)
        ...
        init1 = torch.randn(batch_size, model.init_dim).to(device)
        outputs, outputs_drum = model.generate(init, init1, window_size, events=events[:-1], events_drum=events_drum[:-1],
        ...
        loss2 = loss_function(outputs_drum.view(-1, event_dim), events_drum.view(-1))
        loss = loss1 + loss2
        model.zero_grad()
        loss.backward()
        ...
        print('iter {iteration}, loss: {loss}'.format(iteration = iteration, loss = loss.item()))
        if iteration>10000:
            break
