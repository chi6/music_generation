...
assert os.path.isfile(sess_path), '"{sess_path}" is not a file'.format(sess_path = sess_path)
...
if control is not None:
    if os.path.isfile(control) or os.path.isdir(control):
        if os.path.isdir(control):
            ...
            assert len(files) > 0, 'no file in "{control}"'.format(control = control)
...
if init_zero:
    ...
    init1 = torch.zeros(batch_size, model.init_dim).to(device)
else:
...
with torch.no_grad():
    if use_beam_search:
        ...
        event = torch.tensor(np.asarray([[60]]), dtype=torch.float32)
        drum = torch.tensor(np.asarray([[60]]), dtype=torch.float32)
        outputs, drums = model.generate(init,init1, max_len,None, None,
                                greedy=greedy_ratio,
                                temperature=temperature,
                                verbose=True)

outputs = outputs.cpu().numpy().T # [batch, steps]
drums = drums.cpu().numpy().T
...
n_notes = utils.event_indeces_to_midi_file([outputs[0], drums[0]], path)
