...
DEFAULT_TIME_SHIFT_BINS =1.15 ** np.arange(32) / 65 #1.15 ** np.arange(32) / 65
...
class NoteSeq:
    ...
    @staticmethod
    def from_drum_midi_file(path, *kargs, **kwargs):
        midi = PrettyMIDI(path)
        return NoteSeq.from_drum_midi(midi, *kargs, **kwargs)

    @staticmethod
    def from_drum_midi(midi, programs = DEFAULT_DRUM_PROGRAMS):
    ...
    def to_midi(self, is_drum = False, program=DEFAULT_SAVING_PROGRAM,
                resolution=DEFAULT_RESOLUTION, tempo=DEFAULT_TEMPO):
        midi = PrettyMIDI(resolution=resolution, initial_tempo=tempo)
        inst = Instrument(program, is_drum, 'NoteSeq')
        inst.notes = copy.deepcopy(self.notes)
        midi.instruments.append(inst)
        return midi

    def to_drum_midi_file(self, path, is_drum = True,*kargs, **kwargs):
        self.to_midi(is_drum,*kargs, **kwargs).write(path)

    def to_midi_file(self, path, is_drum = False,*kargs, **kwargs):
        self.to_midi(is_drum,*kargs, **kwargs).write(path)
